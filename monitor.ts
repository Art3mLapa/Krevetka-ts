...
function getFiles(obj: any, currentPath: string = '', files: Record<string, any> = {}): Record<string, any> {
...
function compareJsons(oldJson: any, newJson: any): { added: string[]; changed: string[]; removed: string[] } {
    ...
    Array.from(newPaths).forEach(p => {
        if (!oldPaths.has(p)) {
            added.push(p);
        } else if (JSON.stringify(newFiles[p]) !== JSON.stringify(oldFiles[p])) {
            changed.push(p);
        }
    });
...
function addToTree(root: TreeNode, filePath: string, mark: '+' | '~' | '-'): void {
...
function renderDiffTree(node: TreeNode, id: number = 0): string {
    let html = '';
    const color = node.mark === '+' ? '#a0d468' : node.mark === '~' ? '#ffd700' : node.mark === '-' ? '#ff6b6b' : '#c5c5c5';
    const markStr = node.mark ? `${node.mark} ` : '';
    const hasChildren = node.children.length > 0;
    if (node.isFile) {
        html += `<div class="file ${node.mark ? node.mark : ''}">${markStr}<span style="color: ${color};">${node.name}</span></div>`;
    } else {
...
function renderFullTree(obj: any, currentPath: string = '', id: number = 0): string {
    let html = '';
    if ('File Size' in obj && 'File Hash' in obj && 'File URL' in obj) {
        // файл
        ...
        if (keys.length > 0) {
            if (currentPath === '/') {
                keys.forEach((key, index) => {
                    const newPath = key;
                    html += renderFullTree(obj[key], newPath, id * 10 + index + 1);
                });
            } else {
                html += `<details class="directory">`;
                html += `<summary class="name">${path.basename(currentPath) || '/'}</summary>`;
                html += `<div class="path">/${currentPath.replace(/^\/+/, '')}</div>`;
                keys.forEach((key, index) => {
                    const newPath = currentPath ? `${currentPath}/${key}` : key;
                    html += renderFullTree(obj[key], newPath, id * 10 + index + 1);
                });
...
async function monitor() {
    ...
    let currentDiff = { added: [], changed: [], removed: [] };
    ...
    async function checkAndUpdate() {
        ...
        const localSize = fs.existsSync(torrentFilePath) ? fs.statSync(torrentFilePath).size : 0;
        console.log('[DEBUG] Local file size:', localSize);

        if (localSize === 0 || localSize !== remoteSize) {
            const tempTorrentPath = path.join(torrentDataDir, 'new_torrentData');
            try {
                const dlRes = await axios.get<DownloadResponse>(downloadUrl, { responseType: 'arraybuffer' });
            ...
            const [localHash, tempHash] = await Promise.all([getFileHash(torrentFilePath), getFileHash(tempTorrentPath)]);
            console.log('[DEBUG] Local hash:', localHash);
            console.log('[DEBUG] Temp hash:', tempHash);

            if (localSize === 0 || localHash !== tempHash) {
                // бекап старых файлов
                const oldTorrentPath = path.join(torrentDataDir, 'old_torrentData');
                const oldJsonPath = path.join(projectDir, 'old_stalcraft_data.json');
                if (fs.existsSync(torrentFilePath)) fs.renameSync(torrentFilePath, oldTorrentPath);
                if (fs.existsSync(jsonFilePath)) fs.renameSync(jsonFilePath, oldJsonPath);
                ...
                console.log('[DEBUG] Parsed new torrentData to stalcraft_data.json.');

                // сравнениие двух JSON файлов
                if (fs.existsSync(oldJsonPath)) {
                    const oldJson = JSON.parse(fs.readFileSync(oldJsonPath, 'utf8'));
                    const newJson = JSON.parse(fs.readFileSync(jsonFilePath, 'utf8'));
                ...
                if (fs.existsSync(oldTorrentPath)) fs.unlinkSync(oldTorrentPath);
...
app.get('/krevetka', (req, res) => {
    ...
    if (fs.existsSync(diffFilePath)) {
        const diffLines = fs.readFileSync(diffFilePath, 'utf8').split('\n').filter(line => line.trim());
        if (diffLines.length > 0) {
            ...
            diffHtml = `<div class="changes">${root.children.map((child, index) => renderDiffTree(child, index)).join('')}</div>`;
        }
    }
    const html = `
        <!DOCTYPE html>
        ...
        <body>
            <h1>Файлы игры STALCRAFT ${new Date().toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' })}</h1>
            ${diffHtml}
            <div class="footer">
...
app.get('/explore', (req, res) => {
    ...
    if (fs.existsSync(jsonFilePath)) {
        ...
        treeHtml = `<div class="changes">${renderFullTree(jsonData, '/')}</div>`;
    }
    const html = `
        <!DOCTYPE html>
        <html lang="ru">
        ...
        <body>
            <h1>Обзор файлов STALCRAFT ${new Date().toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric' })}</h1>
            ${treeHtml}
            <div class="footer">
                <h3><a href="https://github.com/Art3mLapa">Site made by krevetka-ts</a></h3>
            </div>
